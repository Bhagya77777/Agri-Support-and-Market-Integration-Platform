...
const FarmerSignup = () => {
  ...
  const handleChange = (e) => {
    ...
    } else if (name === "farmerName" || name === "cropType") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/farmer_login"), 2000);
    } catch (err) {
      setLoading(false);
    ...
    <div className="bg-yellow-50 min-h-screen py-10">
      <div className="container mx-auto max-w-lg shadow-lg rounded-lg border border-yellow-200 p-6">
        <div className="text-center mb-6">
          <h3 className="text-green-700 font-bold text-xl mb-1">
            Create An Account
          </h3>
        ...
        {error && (
          <div className="bg-red-100 text-red-600 border border-red-500 p-3 rounded mb-4">
            {error}
          </div>
        )}
        {success && (
          <div className="bg-green-100 text-green-700 border border-green-500 p-3 rounded mb-4">
            {success}
          </div>
        )}
